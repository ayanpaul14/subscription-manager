...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      const { user, token } = await signInWithGoogle(); // Destructure both user and token

      // Sending data to your real-life backend API
      const res = await api.post("/auth/google", {
        name:  user.name,
        email: user.email,
        uid:   user.uid,
        photo: user.photo,
        token: token // Your backend should verify this token
      });
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center
                    overflow-hidden bg-[#0f0a1e] py-8">
        ...
        <div className="relative p-8 rounded-3xl overflow-hidden" style={{
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="mb-5">
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.97 }}
              className="w-full py-3.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-3 transition duration-200 disabled:opacity-60"
          ...
          {/* Form */}
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4 relative z-20">
            {/* Full Name */}
            <div>
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">Full Name</label>
              <div className="relative">
                <FiUser className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-400 text-base" />
                <input type="text" placeholder="Your Name" {...register("name")} className={inputClass(errors.name)} />
              </div>
              {errors.name && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><FiAlertCircle /> {errors.name.message}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <FiMail className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-400 text-base" />
                <input type="email" placeholder="you@example.com" {...register("email")} className={inputClass(errors.email)} />
              </div>
              {errors.email && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><FiAlertCircle /> {errors.email.message}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-400 text-base" />
                <input type={showPass ? "text" : "password"} placeholder="••••••••" {...register("password")} className={inputClass(errors.password)} />
              ...
              {watchedPassword && (
                <div className="mt-2 flex gap-1">
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} className={`flex-1 h-1 rounded-full ${strength.score >= i ? strength.bg : 'bg-white/10'}`} />
                  ))}
                </div>
              )}
              {errors.password && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><FiAlertCircle /> {errors.password.message}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-400 text-base" />
                <input type={showConfirm ? "text" : "password"} placeholder="••••••••" {...register("confirmPassword")} className={inputClass(errors.confirmPassword)} />
              ...
              {errors.confirmPassword && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><FiAlertCircle /> {errors.confirmPassword.message}</p>}
            </div>

            <motion.button type="submit" disabled={loading}
              whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}
              className="w-full py-3.5 rounded-xl text-sm font-bold text-white shadow-lg mt-4 disabled:opacity-60"
